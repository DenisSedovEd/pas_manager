import { C as vModelText, Ct as inject, Dr as normalizeClass, G as createBaseVNode, Gt as provide, J as createElementBlock, Jn as readonly, K as createBlock, Ln as init_reactivity_esm_bundler, Lt as onMounted, N as Fragment, Nr as toDisplayString, Nt as onBeforeUnmount, St as init_runtime_core_esm_bundler, Tn as withDirectives, Ut as openBlock, W as computed, Yn as ref, Yt as renderList, Zt as resolveComponent, dt as getCurrentInstance, er as toRaw, mr as init_shared_esm_bundler, q as createCommentVNode, qn as reactive, rt as defineComponent, tt as createVNode, u as init_runtime_dom_esm_bundler, vn as watch, w as vShow } from "./runtime-dom.esm-bundler-CtBCU7J0.js";
import "./vue.js";
//#region node_modules/vue3-emoji-picker/dist/emoji-picker.es.js
init_reactivity_esm_bundler(), init_runtime_core_esm_bundler(), init_shared_esm_bundler(), init_runtime_dom_esm_bundler();
...
var emojis = {
	...
	symbols: [
		...
		{
			n: ["clockwise rightwards and leftwards open circle arrows with circled one overlay", "repeat_one"],
			u: "1f502"
		},
		{
...
async function initialize() {
	(await openDB(DB_KEY, DB_VERSION, { upgrade(db2, oldVersion) {
		if (!db2.objectStoreNames.contains(STORE_KEY)) db2.createObjectStore(STORE_KEY, {
			keyPath: "id",
			autoIncrement: true
...
async function getRecentEmojis() {
	return await (await openDB(DB_KEY, DB_VERSION)).transaction(STORE_KEY, "readonly").objectStore(STORE_KEY).getAll();
}
function Store() {
	const state = reactive({
		...
		get disabled() {
			...
			if (!this.options.displayRecent) disabled = ["recent", ...disabled];
			return disabled;
		},
		get groups() {
	...
	function initialize2() {
		if (state.options.displayRecent) setInitialRecentEmojis();
	}
	async function getRecent() {
		var _a;
		let recent2 = await getRecentEmojis();
		if (Array.isArray(recent2) && recent2.length) recent2 = JSON.parse(((_a = recent2[0]) == null ? void 0 : _a.value) || "");
		else recent2 = [];
		return recent2;
	}
	...
	async function updateLocalStore() {
		(await openDB(DB_KEY, DB_VERSION)).transaction(STORE_KEY, "readwrite").objectStore(STORE_KEY).put({
			id: 0,
	...
	const updateSelect = (emoji) => {
		...
		if (index2 > 0) state.recent.splice(index2, 1);
		if (index2 === 0) return;
		state.recent = [{
			u: emoji.u,
			n: toRaw(emoji.n)
...
function popperGenerator(generatorOptions) {
	...
	return function createPopper2(reference2, popper2, options) {
		if (options === void 0) options = defaultOptions2;
		var state = {
			placement: "bottom",
			orderedModifiers: [],
...
function filterEmojis(emojis2, keyword, skinTone, disabledGroups = []) {
	...
	Object.keys(emojis2).forEach((key) => {
		if (disabledGroups.includes(key)) return;
		const _emojis = [];
		emojis2[key].forEach((emoji) => {
			...
			if (emoji[EMOJI_NAME_KEY][0].includes(keyword.toLocaleLowerCase())) {
				...
				if (skinTone !== SKIN_TONE_NEUTRAL && Array.isArray(emoji[EMOJI_VARIATIONS_KEY])) {
					...
					if (v_index !== -1 && emoji[EMOJI_VARIATIONS_KEY]) result = emoji[EMOJI_VARIATIONS_KEY][v_index];
				}
				return _emojis.push({
					...emoji,
		...
		if (_emojis.length) _emojiData[key] = _emojis;
	});
	return _emojiData;
}
function isMac() {
	var _a;
	return (((_a = navigator == null ? void 0 : navigator.userAgentData) == null ? void 0 : _a.platform) || (navigator == null ? void 0 : navigator.platform) || "unknown").toUpperCase().indexOf("MAC") !== -1;
}
...
var _sfc_main$4 = defineComponent({
	...
	setup() {
		...
		if (state.options.additionalGroups) Object.keys(state.options.additionalGroups).map((k) => {
			if (state.options.groupNames[k]) groupNames[k] = state.options.groupNames[k];
			else groupNames[k] = snakeToCapitalizedCase(k);
		});
		...
		function handleError(event, unicode) {
			...
			if (button) button.innerHTML = `<span>${unicodeToEmoji(unicode)}</span>`;
		}
		watch(() => state.activeGroup, () => {
			var _a;
			const target = (_a = bodyInner.value) == null ? void 0 : _a.querySelector("#" + state.activeGroup);
			if (target) target.parentNode.scrollTop = target.offsetTop - target.parentNode.offsetTop;
		});
		return {
			emojis: emojis2,
			bodyInner,
...
function _sfc_render$4(_ctx, _cache, $props, $setup, $data, $options) {
	return openBlock(), createElementBlock("div", _hoisted_1$3, [createBaseVNode("div", {
		...
		return openBlock(), createElementBlock("div", {
		...
		}, toDisplayString(_ctx.groupNames[key]), 3)), [[vShow, _ctx.emojis[key]]]) : createCommentVNode("", true), withDirectives(createBaseVNode("div", _hoisted_3$3, [(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.emojis[key], (emoji) => {
			...
			}, null, 40, _hoisted_6$2))], 40, _hoisted_4$3);
		}), 128))], 512), [[vShow, _ctx.emojis[key]]])], 8, _hoisted_2$3);
	}), 128)) : (openBlock(), createElementBlock("span", _hoisted_7$1, " No emoji has been found! "))], 2)]);
}
...
var _sfc_main$3 = defineComponent({
	...
	setup(props) {
		...
		const groups = [...state.groups, ...Object.keys(state.options.additionalGroups).map((g) => ({
			key: g,
...
function _sfc_render$3(_ctx, _cache, $props, $setup, $data, $options) {
	return _ctx.hasGroupIcons || _ctx.hasSearch ? (openBlock(), createElementBlock("div", _hoisted_1$2, [
		_ctx.hasGroupIcons ? (openBlock(), createElementBlock("div", _hoisted_2$2, [(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.orderedGroups, (group) => {
			return openBlock(), createElementBlock("button", {
				key: group.key,
				type: "button",
				class: normalizeClass(["v3-group", { "v3-is-hidden": !_ctx.icons[group.key] }]),
				...
				alt: ""
			}, null, 8, _hoisted_5$2)], 8, _hoisted_4$2)], 10, _hoisted_3$2);
		}), 128))])) : createCommentVNode("", true),
		_ctx.hasGroupIcons && _ctx.hasSearch ? (openBlock(), createElementBlock("div", _hoisted_6$1)) : createCommentVNode("", true),
		_ctx.hasSearch ? (openBlock(), createElementBlock("div", _hoisted_7, [withDirectives(createBaseVNode("input", {
			"onUpdate:modelValue": _cache[0] || (_cache[0] = ($event) => _ctx.searchValue = $event),
			type: "text",
			placeholder: _ctx.placeholder
		}, null, 8, _hoisted_8), [[vModelText, _ctx.searchValue]])])) : createCommentVNode("", true)
...
var _sfc_main$2 = defineComponent({
	...
	setup() {
		...
		const emoji = computed(() => {
			return {
				...state.emoji,
				src: "https://cdn.jsdelivr.net/npm/emoji-datasource-apple@6.0.1/img/apple/64/" + state.emoji[EMOJI_RESULT_KEY] + ".png"
			};
		});
...
function _sfc_render$2(_ctx, _cache, $props, $setup, $data, $options) {
	...
	}, [createBaseVNode("div", _hoisted_1$1, [createBaseVNode("span", { class: normalizeClass([_ctx.platform, "v3-icon"]) }, [_ctx.native || _ctx.hasError ? (openBlock(), createElementBlock("span", _hoisted_2$1, toDisplayString(_ctx.unicodeToEmoji(_ctx.emoji.r)), 1)) : (openBlock(), createElementBlock("img", {
		key: 1,
		alt: _ctx.unicodeToEmoji(_ctx.emoji.r),
		src: _ctx.emoji.src,
		onError: _cache[0] || (_cache[0] = ($event) => _ctx.hasError = true)
	}, null, 40, _hoisted_3$1))], 2), createBaseVNode("span", _hoisted_4$1, " :" + toDisplayString(_ctx.emoji[_ctx.EMOJI_NAME_KEY][1] || _ctx.emoji[_ctx.EMOJI_NAME_KEY][0]) + ": ", 1)]), _ctx.hasSkinTones ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [createBaseVNode("button", {
		type: "button",
		class: "v3-tone",
		onClick: _cache[1] || (_cache[1] = (...args) => _ctx.toggleSkinToneState && _ctx.toggleSkinToneState(...args))
	}, [createBaseVNode("span", _hoisted_5$1, toDisplayString(_ctx.skinToneText), 1), createBaseVNode("span", { class: normalizeClass(`v3-icon v3-tone-${_ctx.stateSkinTone}`) }, null, 2)]), createBaseVNode("div", { class: normalizeClass([_ctx.skinTone ? "v3-is-open" : "", "v3-skin-tones"]) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(_ctx.SKIN_TONES, (tone) => {
		return openBlock(), createElementBlock("button", {
			key: tone,
			type: "button",
	...
	}), 128))], 2)], 64)) : createCommentVNode("", true)], 32);
}
var _sfc_main$1 = defineComponent({
	...
	components: {
		Header,
		Body,
		Footer: /* @__PURE__ */ _export_sfc(_sfc_main$2, [["render", _sfc_render$2]])
	},
	props: {
		type: {
			type: String,
	...
	setup(props, { emit }) {
		...
		function updateCursor() {
			var _a;
			if (elem.value) cursor = ((_a = elem.value) == null ? void 0 : _a.selectionEnd) || -1;
		}
		function clickListener(event) {
			var _a;
			if (!((_a = event.target) == null ? void 0 : _a.closest(".v3-input-picker-wrap")) && open.value) open.value = false;
		}
		function setupPopper() {
			if (button.value && picker.value && isInputType) {
				let offset2 = state.options.offset;
				if (typeof offset2 !== "number") offset2 = 6;
				createPopper(button.value, picker.value, {
					placement: "bottom-end",
					modifiers: [{
						name: "offset",
						options: { offset: [0, offset2] }
					}]
				});
...
function _sfc_render$1(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return _ctx.isInputType ? (openBlock(), createElementBlock("div", _hoisted_1, [createBaseVNode("div", _hoisted_2, [_ctx.type === "input" ? (openBlock(), createElementBlock("input", {
		key: 0,
		ref: "elem",
		value: _ctx.input,
		type: "text",
	...
	}, null, 40, _hoisted_4)), createBaseVNode("div", { class: normalizeClass(["v3-input-picker-wrap", _ctx.open ? "v3-picker-is-open" : ""]) }, [createBaseVNode("button", {
		ref: "button",
		type: "button",
		...
		src: _ctx.face,
		alt: ""
	}, null, 8, _hoisted_5)], 512), createBaseVNode("div", {
		ref: "picker",
...
var _sfc_main = defineComponent({
	name: "Picker",
	components: { PickerRoot: /* @__PURE__ */ _export_sfc(_sfc_main$1, [["render", _sfc_render$1]]) },
	props: {
		native: {
			type: Boolean,
			default: false
